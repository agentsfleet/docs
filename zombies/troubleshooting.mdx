...
```bash
zombiectl install --from ./my-agent --json 2>&1 | tail -5
```
...
Run terminated with `UZ-ZMB-001`. The `daily_dollars` or `monthly_dollars` ceiling in `TRIGGER.md` was hit.
...
2. **Tenant wallet** — every workspace's runs debit the same pool. When it hits zero, all runs across all workspaces stop. Top up at `app.usezombie.com/billing`. See [Billing → Budgets](/billing/budgets).
