...
Some zombie actions — sending money, merging a PR, posting to a public channel — should not happen without a human in the loop. The **approval gate** lets a zombie pause mid-run, emit an approval request, and wait for a human to click approve or deny in the dashboard.
