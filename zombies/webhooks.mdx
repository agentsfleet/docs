...
<Note>
  **Dedicated routes for some providers.** GitHub events have a tailored payload-normalizer route at `POST /v1/webhooks/{zombie_id}/github`. Clerk-shaped (Svix) webhooks land at `POST /v1/webhooks/svix/{zombie_id}` for Svix's multi-signature rotation scheme. Both still use `secret_ref` from the workspace vault. Use these only if you need the specific normalization — the source-addressed `POST /v1/webhooks/{zombie_id}/{source}` route plus generic HMAC works for everyone else.
</Note>
