...
If an agent references a credential that is not present in the vault when `zombiectl install --from <path>` is called, install fails with a clear error listing the missing names.
