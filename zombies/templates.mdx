---
title: "Templates"
description: "The shipped sample directory — pre-built agents you can install with one command."
---
...
A template is just a directory with `SKILL.md` and `TRIGGER.md`. See [Authoring an agent](/agents/authoring) for the file references; install with:
...
Templates are markdown — commit `my-agent/` to git, clone it on a new machine, `agentsfleet install --from .`. Credentials never live in the template; they live in the workspace vault. Template-safe to share publicly; credentials are not.
