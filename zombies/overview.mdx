...
4. **Killed.** `zombiectl kill <zombie_id>` marks the zombie terminal. The row, history, and webhook URL persist but no new events are processed. State is checkpointed; nothing on the event stream is lost. Re-install with the same `SKILL.md` produces a **new** `zombie_id`. To hard-remove the row and webhook URL, follow with `zombiectl delete`.
