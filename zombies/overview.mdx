...
A zombie moves through five observable states.
...
3. **Stopped.** `zombiectl stop <zombie_id>` halts the running session — new events stop dispatching, the in-flight stage finishes cleanly. Reversible with `zombiectl resume`. Use this for "pause while I debug upstream"; reach for `kill` only when you mean *terminal*.
4. **Killed.** `zombiectl kill <zombie_id>` marks the zombie terminal. The row, history, and webhook URL persist but no new events are processed. State is checkpointed; nothing on the event stream is lost. Re-install with the same `SKILL.md` produces a **new** `zombie_id`.
5. **Deleted.** `zombiectl delete <zombie_id>` hard-removes the zombie. The row, its `zombie_id`, and its webhook URL are gone — the webhook URL starts returning `UZ-WH-001` for any new POSTs. The activity-stream history is retained for the workspace's retention window per [Observability](/cli/zombiectl#observability); replays after delete reference the prior `zombie_id` as a historical record only. You must `kill` the zombie before `delete` will accept it.
...
- **Webhook namespace** — every zombie in a workspace gets one unique URL per declared trigger under `https://api.usezombie.com/v1/webhooks/{zombie_id}/{source}` (one entry per `triggers[].source` in `TRIGGER.md`).
