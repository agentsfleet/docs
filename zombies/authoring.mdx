...
x-usezombie:                # all runtime config nests under this key
  triggers:                 # array, 1–8 entries; one zombie can wake on many sources
...
|---|---|---|---|
| `webhook` | An external HTTP `POST` to `https://api.usezombie.com/v1/webhooks/{zombie_id}/{source}` with a signed body. | Most production zombies — anything driven by a third-party event (GitHub, Slack, Linear, Jira, etc.). | `source` (label, also selects the URL) + `events` (optional whitelist) + `signature` (HMAC config). See [Webhooks](/zombies/webhooks). |
| `cron` | A self-scheduled timer the zombie sets via the `cron_add` tool. | Periodic health checks, drift sweeps, scheduled cleanups. | None at the trigger level — schedules live inside the reasoning loop. At most one `cron` entry per zombie. |
...
A zombie can declare up to eight triggers and wake on any of them — e.g. one webhook from GitHub plus a cron sweep every 30 minutes. Any zombie can also be steered via `zombiectl steer` regardless of declared triggers — steer always works. The `api` type is for zombies whose *primary* surface is steer (no inbound webhook).

<Note>**Long-run continuations are not a trigger type.** When a run runs out of context, the runtime re-enqueues the same event with `actor=continuation:<original_actor>` automatically. You don't declare this in `TRIGGER.md`; it happens regardless of which trigger type woke the original run. See [Context lifecycle](/concepts/context-lifecycle).</Note>
