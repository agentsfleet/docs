...
|---|---|---|
| `memory_store` | Upsert a distilled fact (key + category + body + tags). | End of every run where the zombie learned something worth keeping. |
...
Self-scheduled future runs. The zombie sets these from inside the reasoning loop — useful for follow-ups, periodic health checks, and deferred work.
...
`zombiectl install --from <path>` accepts the `tools:` list as-is — there's no catalogue check at install time. Unknown names surface later, when the executor builds tools for a run: each unrecognized name gets logged as `UZ-TOOL-005` (Unknown tool) and silently skipped. The agent runs with the remaining tools.
