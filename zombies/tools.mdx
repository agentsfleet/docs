...
description: "Every tool an agent can declare in `TRIGGER.md`, what it does, and when to reach for it."
---

A **tool** is a named verb the agent inside a run may invoke. Tools are declared in `TRIGGER.md` under `x-agentsfleet.tools:` — anything not on the list is unreachable, even if the model tries to call it. This page is the canonical catalogue.
...
23 tools are user-callable today. They group into nine categories below. For the file format, validation rules, and frontmatter, see [Authoring an agent](/agents/authoring).
...
Tools are the *what* (verbs the agent may invoke). Credentials are the *what-with* (named secrets the tool bridge substitutes into outbound calls — see [Credentials](/agents/credentials)). The network allowlist is the *to-where* (hostnames the sandbox firewall lets traffic reach).
...
`agentsfleet install --from <path>` accepts the `tools:` list as-is — there's no catalogue check at install time. Unknown names surface later, when the runner builds tools for a run: each unrecognized name gets logged as `UZ-TOOL-005` (Unknown tool) and silently skipped. The agent runs with the remaining tools.

The practical consequences:

- **Typos pass install.** Declaring `http_requesst:` (typo) won't fail `agentsfleet install --from`. Catch this by tailing `agentsfleet logs <agent_id>` after the first event and looking for the skip warning, or by spotting tool calls in the activity stream that don't match what your `SKILL.md` prose expects.
...
- [Authoring an agent](/agents/authoring) — full `TRIGGER.md` schema.
- [Memory](/memory) — the four memory categories and how recall/store fit a run.
- [Credentials](/agents/credentials) — adding the secrets the tool bridge substitutes.
- [Webhooks](/agents/webhooks) — wiring an external system to your agent.
