...
- Otherwise the **stored token** written by `agentsfleet login` to `~/.config/agentsfleet/credentials.json` is used.
...
| `AGENTSFLEET_STATE_DIR` | Directory for local CLI state (login token, telemetry, session). | `~/.config/agentsfleet` |
