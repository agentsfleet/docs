...
Inspect the stored credential without re-authenticating. Resolves the token source (file vs `ZOMBIE_API_TOKEN` env vs none), decodes the JWT claims (`iss`, `aud`, `sub`, `metadata.tenant_id`, `metadata.role`, `exp`), and probes `GET /v1/tenants/me/billing` to check the token is still accepted by the server.
...
echo "check the latest deploy event" | zombiectl steer zmb_2041 --tty
```

When you invoke `zombiectl steer <zombie_id>` from a terminal (TTY) and omit the message, `zombiectl` opens a read-eval-print loop (REPL). Each submitted line posts one steer message, streams the reply, then returns to the prompt. Press `Ctrl-D` to exit cleanly, or `Ctrl-C` to cancel the active stream and exit.
