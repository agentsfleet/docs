...
```bash
agentsfleet stop   zmb_2041   # pause; resumable
agentsfleet resume zmb_2041   # back to active
agentsfleet kill   zmb_2041   # terminal; webhook still 200s but routes nowhere new
agentsfleet delete zmb_2041   # hard-delete; webhook URL starts returning UZ-WH-001
```
...
```bash
agentsfleet logs zmb_2041
agentsfleet logs zmb_2041 --limit 50 --cursor <next_cursor>
agentsfleet logs zmb_2041 --json
```
...
```bash
agentsfleet events zmb_2041 --actor 'webhook:*' --since 2h
agentsfleet events zmb_2041 --actor 'steer:*' --since 7d --limit 100
agentsfleet events zmb_2041 --json | jq 'select(.error)'
```
...
```bash
agentsfleet steer zmb_2041
agentsfleet steer zmb_2041 "morning health check — anything red?"
echo "check the latest deploy event" | agentsfleet steer zmb_2041
printf "check the latest deploy event\ncheck memory usage\n" | agentsfleet steer zmb_2041 --tty
```
...
```bash
agentsfleet workspace use ws_abc123
```
...
```bash
agentsfleet workspace delete ws_abc123
```
...
```bash
agentsfleet agent add --workspace ws_abc --agent zm_xyz --name my-agent
```
...
```bash
agentsfleet agent list --workspace ws_abc
```

### `agentsfleet agent delete <agent-id>`

Revoke an external agent key.

```bash
agentsfleet agent delete --workspace ws_abc ag_01JABC...
```
...
```bash
agentsfleet grant list --agent zm_01JABC...
```
...
```bash
agentsfleet grant delete --agent zm_01JABC... gr_01JXYZ...
```
