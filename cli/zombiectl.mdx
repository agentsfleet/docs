...
- `--timeout-sec`, `--poll-ms` — tune the session poll loop.
- `--force` — skip the "you're already logged in as `<name>`, replace?" prompt.
- `--no-input` — fail fast instead of prompting for the verification code (use `ZMB_TOKEN` env var for non-interactive shells instead).

The login session lives 5 minutes; an unfinished approval expires automatically. Wrong verification code attempts cap at 5 per session — a sixth wrong code aborts the session and you re-run `zombiectl login`.
...
Does **NOT** revoke your active session token — JWTs are short-lived (~15 min) and expire on their own. Does **NOT** touch `ZMB_TOKEN` / `ZOMBIE_TOKEN` environment variables — if either is set, subsequent commands continue to use that token until you unset it.

### `zombiectl auth status`

Inspect the stored credential without re-authenticating. Resolves the token source (file vs `ZOMBIE_API_TOKEN` env vs none), decodes the JWT claims (`iss`, `aud`, `sub`, `metadata.tenant_id`, `metadata.role`, `exp`), surfaces the `token_name` label persisted at login, and probes `GET /v1/tenants/me/billing` to check the token is still accepted by the server.
