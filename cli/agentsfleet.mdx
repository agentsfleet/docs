...
Four verbs control an fleet's lifecycle, in increasing order of irreversibility:
...
A read-only window into an fleet's durable memory. There is no write verb — the tenant memory plane is read-only by architecture; fleets write their own memory during runs.
...
Substring-search keys and content across an fleet's memory.
...
List pending and approved grants for an fleet.
