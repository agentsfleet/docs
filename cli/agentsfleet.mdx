...
| `agentsfleet secret create <NAME> --provider <ID> --api-key <KEY> --model <MODEL>` | Stores a model-provider credential. `--provider` accepts only ids the runtime can dial; an unknown id exits with code `2` and lists them. `--provider openai-compatible` adds a required `https` `--base-url` and makes `--api-key` optional. |
