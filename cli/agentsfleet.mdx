...
| Command | Effect |
|---|---|
| `agentsfleet schedule add` | Adds a schedule that wakes a fleet on a cron expression. |
| `agentsfleet schedule list` | Lists the schedules a fleet holds. |
| `agentsfleet schedule update` | Changes an existing schedule. |
| `agentsfleet schedule rm` | Removes one schedule. |
| `agentsfleet schedule status` | Shows a schedule and its state at the upstream scheduler. |
| `agentsfleet schedule sync` | Re-sends the schedule to the upstream scheduler. |

Run `agentsfleet schedule sync` after `UZ-SCHED-004`, the code the server returns when the upstream scheduler did not confirm a change.

| Option | Effect | Default | Unit or range |
|---|---|---|---|
| `--cron <EXPRESSION>` | Sets when the schedule fires. | Unset | Five-field cron expression |
| `--message <TEXT>` | Sets the message the fleet receives on each fire. | Unset | Text |
| `--timezone <TIMEZONE>` | Reads the cron expression in this zone. | Unset | Supported region name |
| `--status <STATE>` | Sets whether the schedule fires. | Unset | `active` or `paused` |
| `--workspace <WORKSPACE_ID>` | Targets a workspace other than the active one. | Active workspace | Workspace identifier |

A fleet holds at most 32 schedules. Run `agentsfleet schedule <SUBCOMMAND> --help` for the arguments one subcommand takes.
