...
| `agentsfleet status` | Shows every fleet in the active workspace, including how many approvals each one is waiting on. |
...
| Command | Effect |
|---|---|
| `agentsfleet schedule add <FLEET_ID> --cron <EXPRESSION> --message <TEXT>` | Adds a schedule that wakes a fleet on a cron expression. |
...
| `agentsfleet schedule rm <FLEET_ID> <SCHEDULE_ID>` | Removes one schedule. |
| `agentsfleet schedule status <FLEET_ID> <SCHEDULE_ID>` | Shows a schedule and its state at the upstream scheduler. |
