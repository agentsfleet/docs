...
With the CLI installed, sign in and install a fleet from the library catalogue:

```bash
agentsfleet login                              # browser approval + verification code
agentsfleet library                            # browse the catalogue
agentsfleet install --library github-pr-reviewer
```

Authored your own `SKILL.md` + `TRIGGER.md`? Onboard it as a template (via the dashboard or the API), then install it by id with `agentsfleet install --library <id>`. To push local edits onto an already-installed fleet, use `agentsfleet fleet update <fleet_id> --from <path>`. See [Authoring a fleet](/fleets/authoring).
