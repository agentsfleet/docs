...
`GET /v1/users/me` returns the person a credential belongs to, their tenant, which credential class proved them, and the scopes they hold. It requires no scope of its own, so it is also the cheapest way to check that a credential still works.
...
```text
{"items":<varies>,"total":<varies>,"next_cursor":<varies>}
```
