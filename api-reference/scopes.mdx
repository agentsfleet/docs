...
<Note>
If a request returns [`UZ-AUTH-022`](/api-reference/error-codes#UZ-AUTH-022) (Insufficient scope), the response's `detail` field names the exact scope your token is missing. Ask a workspace admin to grant it — see [API keys](/api-reference/introduction) for how a tenant API key's scopes are set.
</Note>
