...
| `library:write` | Adds an entry to a workspace library, and removes one the workspace onboarded. | None |
...
`agentsfleet approvals` spends the approval scopes from a terminal or a script. Listing needs `approval:read`, and approving or denying needs `approval:resolve`.
