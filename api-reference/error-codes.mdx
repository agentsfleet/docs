...
| `UZ-VAULT-003` | 404 | Credential not found | No credential matches this name in the workspace. Returned by `PATCH /credentials/{name}` (key rotation) and other by-name credential lookups; list the workspace credentials to find a valid name, or create it first. |
