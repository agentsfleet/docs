...
<Update label="May 17, 2026" tags={["Server", "Performance"]}>
  ...
  `redis_client.zig` previously held one `std.Thread.Mutex` across every command's full write+read round-trip, hard-capping throughput at ~40 ops/sec/connection regardless of how many producer threads issued XADDs. The mutex is gone. Concurrent producers acquire their own pooled connection from `Pool` (default `max_idle=8`, `eager_min=2`); long-lived blocking consumers — the watcher's `XREADGROUP zombie:control`, per-zombie worker `XREADGROUP zombie:{id}:events`, SSE `SUBSCRIBE zombie:{id}:activity` — hold dedicated connections outside the pool by design. Pooling those would exhaust the pool at the 9th zombie.

  - **`Pool` API in `src/queue/redis_pool.zig`** — `acquire`/`release(ok)` with a health flag, `SinglyLinkedList` of idle connections, env knobs `REDIS_POOL_MAX_IDLE` (8) and `REDIS_POOL_EAGER_MIN` (2). `release(false)` marks the connection poisoned; the pool drops it and dials a fresh one on the next acquire.
  - **Per-connection state machine** — `active` → `poisoned` → `closing` → `closed` with audited transitions; misuse fires `std.debug.assert` in debug builds. `Role` enum (`pooled` / `blocking_consumer` / `subscriber`) is the correctness boundary between request-path and long-lived consumers.
  - **Typed error surface in `redis_errors.zig`** — `RedisError` set + `isResumable()` predicate over an exhaustive switch. Adding a new error variant is a compile error until classified.
  - **Request-path read timeout** — `REDIS_REQUEST_TIMEOUT_MS` (default 5000ms) installs `SO_RCVTIMEO`; timeouts surface as `error.RedisRequestTimeout` (non-resumable, connection closed). A misparsed env value fails the boot with `UZ-STARTUP-ENV-CHECK`.
  - **Operator-visible server error replies** — `READONLY` / `BUSYGROUP` / `WRONGTYPE` reply bodies log a `redis_command_err_reply` warn line carrying the server text before being mapped to `error.RedisCommandError`. Underlying cause is visible in logs, not flattened into `ERR_INTERNAL_OPERATION_FAILED`.
  - **Pool metrics on `/metrics`** — 8 new Prometheus series: `zombie_redis_pool_dials_total`, `zombie_redis_pool_overflow_dials_total`, `zombie_redis_pool_poisoned_connections_total`, `zombie_redis_pool_reconnects_total`, `zombie_redis_pool_forced_closes_total`, `zombie_redis_pool_acquire_timeouts_total`, `zombie_redis_pool_idle`, `zombie_redis_pool_acquire_wait_ns_p99`.
  - **Subscriber unification** — `redis_pubsub.zig` deleted; one `Subscriber` accepts `InitOptions { read_timeout_ms: ?u32 = null }`. Production passes null (block forever); test harnesses pass a budget.

  Operator note — env knobs (`REDIS_POOL_MAX_IDLE`, `REDIS_POOL_EAGER_MIN`, `REDIS_REQUEST_TIMEOUT_MS`) take effect at process start; rotate `zombied` to pick up changes.
</Update>
