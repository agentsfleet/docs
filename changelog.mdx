...
<Update label="Sep 2, 2026" tags={["What's new", "Observability"]}>
  ...
  - **Lost telemetry is counted, never retried** — spans in a batch that failed to send and metric collection cycles that failed to send are counted in the process. A resent window would double-count delta families.
