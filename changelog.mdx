...
<Update label="May 12, 2026" tags={["UI", "Bug fixes", "Testing"]}>
  ...
  - **`presentError({errorCode, message, action})` is the single entry point for dashboard error rendering.** Curated `UZ-XXX-NNN` codes (registry: [`api-reference/error-codes`](/api-reference/error-codes)) map to a title + body the operator can act on — `UZ-AUTH-006` reads "Your session expired. Sign in again to keep going." instead of "Not authenticated"; `UZ-ZMB-009` reads "We couldn't find that zombie. It may have already been deleted — refresh the list." instead of "Internal Server Error". Eight codes ship today and the helper grows organically as the dashboard surfaces new ones. Useless server `"Failed to …"` messages are detected and replaced rather than concatenated.
