...
<Update label="Sep 24, 2026" tags={["What's new", "Bug fixes", "API", "CLI", "Integrations"]}>
  ...
  - **`POST /v1/connectors/slack/events`** — answers `200` with `{"event_id", "replayed"}` for an accepted mention, and `{"ignored": "<reason>"}` otherwise: `unreadable_body`, `bot_message`, `team_not_mapped`, or `unsupported_event`.
