...
<Update label="Jun 10, 2026" tags={["What's new", "API"]}>
  ...
  Platform admins can now cordon, drain, or revoke a runner from the Runners page, then inspect that runner's activity history without exposing runner tokens. Cordon stops new work, drain waits for active leases to finish and then marks the runner drained, and revoke immediately blocks runner-plane authentication.
  ...
  - **Activity history** — Each runner has an activity view backed by `fleet.runner_events`, covering registration, online/offline, lease, and operator-state events.
