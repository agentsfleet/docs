...
<Update label="Sep 11, 2026" tags={["API", "UI"]}>
  ...
  Each frame a fleet publishes on its live tail now carries `events_processed` and `budget_used_nanos` as an absolute snapshot, and the workspace stream's `hello` carries a `counters` map keyed by fleet id. The Fleets wall assigns each tile's event count and spend from whichever frame arrived last and never adds to them, so a dropped, duplicated or late frame cannot leave a tile wrong. A tile is correct before its first event: the `hello` reports where each fleet stands at subscribe time.
