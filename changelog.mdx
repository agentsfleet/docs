...
<Update label="Sep 21, 2026" tags={["API", "CLI", "Dashboard", "What's new"]}>
  ## Onboarding a Fleet library is no longer a one-way door

  A workspace can list the Fleet libraries it onboarded and remove one. Until now nothing removed a row from `core.tenant_fleet_library`, so every entry a workspace ever added stayed in the install gallery. Fleets installed from an entry are untouched by its removal: an install copies the bundle onto the fleet, and the fleet runs from its own copy.

  ## What's new

  - **`GET /v1/workspaces/{workspace_id}/library-entries` is the workspace's own collection** — its entries alone, never a platform row, paged on its own cursor. The merged gallery at `/v1/workspaces/{workspace_id}/fleet-libraries` is unchanged, and `agentsfleet install --library` still resolves against it.
  - **`DELETE /v1/workspaces/{workspace_id}/library-entries/{entry_id}` removes one entry** — `204`, and repeatable. An identifier already gone answers `204`, and so does one naming another workspace's entry, so neither reply tells you whether the entry exists somewhere else. It needs the `library:write` scope.
  - **`agentsfleet library list` and `agentsfleet library delete <ENTRY_ID>` are the terminal half** — `list` prints the identifier `delete` takes. Bare `agentsfleet library` still prints the full gallery, unchanged.
  - **`agentsfleet library add` is now `agentsfleet library create`** — the group spoke a dialect of its own, `add` where every other collection says `create`. No alias is kept: `library add` answers as the unrecognised subcommand it now is, and `agentsfleet library --help` lists the verbs that work. Scripts pinning the old spelling need one edit.
  - **`agentsfleet connector status` is now `agentsfleet connector show`** — a single-resource read is called `show` everywhere else on this surface, and the command's own help already said "Show connector state".
  - **`agentsfleet auth status` is removed; `agentsfleet whoami` answers for it** — the two overlapped on everything but one field. `whoami` now reports `source`, naming whether the live credential came from the `environment` or a `file`, which is the confusing case worth printing: an exported service key silently outranks a stored login, so the terminal looks signed in as one identity and acts as another. `agentsfleet doctor` still reports reachability and workspace binding.
  - **The dashboard grows a Library page** — under the workspace sidebar, listing what this workspace owns, with a confirmation that names the entry before it goes.
  - **Removal is permanent** — no marker row, no hidden state. Onboard the same bundle again and it arrives as a new entry with a new identifier.
  - **No new error code** — a malformed `entry_id` is a plain `400`, a cursor from the wrong collection is `UZ-LIBRARY-001`, and a database failure is `UZ-LIBRARY-006`.
