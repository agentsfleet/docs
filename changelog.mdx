...
<Update label="Sep 25, 2026" tags={["Dashboard", "What's new", "Bug fixes", "API"]}>
  ## Fleet chat shows replies as they arrive

  You can send another message while a fleet is working. The dashboard keeps
  each accepted message in order, shows when it is queued, and adds the reply
  as it arrives.

  Update each `agentsfleet-runner` to 0.51.0 along with the control plane. An
  older runner still completes runs, but its replies appear only when the run
  finishes.

  ## What's new

  - **Reasoning and tool activity have their own place** — the chat keeps the final reply readable while showing progress during a run.
  - **Reasoning text is lighter than the reply** — the answer reads first, and the reasoning still meets the Web Content Accessibility Guidelines (WCAG) AA contrast level.
  - **Reply text arrives in smaller steps** — the dashboard receives new text every 200 milliseconds instead of every second.
  - **`event_complete` carries the final reply** — the fleet stream's completion frame includes `final_reply` when the reply is at most 64 KiB. Without it, read the event.
  - **A failed event carries a copyable diagnostic** — open its event details and click **Copy diagnostic** to capture its identifier and recorded cause for an operator.

  ## Bug fixes

  - **A stopped reply has a plain message** — chat no longer displays a runner error name as if it explained what happened. The cause remains available in the event diagnostic.
  - **Streaming tool calls reach the fleet** — a model that asks to use a tool while streaming can complete the tool call and continue its answer.
  - **A brief network drop no longer fails a run before it starts** — the runner retries the fleet bundle download up to 3 times within 5 seconds.
  - **A failed bundle download no longer blames the fleet's instructions** — chat now says "The runner refused this run before the fleet started."
</Update>

<Update label="Sep 24, 2026" tags={["What's new", "Bug fixes", "API", "CLI", "Integrations"]}>
  ## Mention a fleet in Slack and it answers in the thread

  A fleet attached to a Slack channel now answers when someone mentions `agentsfleet` there, and posts its answer as a reply in the same thread. Until now the Slack route acknowledged every mention and dropped it. [Answer mentions in Slack](/fleets/slack) walks through the setup.

  ## What's new

  - **Attach a fleet at install** — `--slack-channel <CHANNEL_ID>` writes a `mention` trigger for that channel into the fleet's `TRIGGER.md`. One fleet answers in one channel.
  - **The fleet reads the thread** — it sees the first message and the latest 19 replies, each cut at 2,000 characters and 16,000 in total. In a private channel, or when Slack takes longer than 1.5 seconds, it answers from the mention alone.
  - **Name a fleet to pick it** — start the message with a fleet's name when the channel has several. An unnamed mention with several candidates gets a notice that lists their names.
  - **Each channel gets its own fleet** — the first mention no attached fleet takes creates `slack-channel-<TEAM_ID>-<CHANNEL_ID>`. It has no tools, a budget of 1.00 dollars a day, and memory scoped to the channel. It replies with the command that attaches a fleet.
  - **The fleet page shows the channel** — a fleet attached to a channel lists a **Chat mention** trigger with its channel identifier.
  - **Write fleets answer only when named** — a fleet with `repository_access: write` opens at most one draft pull request per mention. The fleet cannot merge that pull request, mark it ready for review, or change a workflow.
  - **Fleets read their Continuous Integration (CI) runs** — a GitHub-bound fleet's token now reads workflow runs and jobs (`actions: read`), and check-run annotations (`checks: read`) where the GitHub App installation holds that permission. A permission the installation lacks is left out of the token, and the mint still succeeds.

  ## API reference

  - **`POST /v1/workspaces/{workspace_id}/fleets`** — accepts an optional `slack_channel_id`, such as `C0123456789`, and adds the `mention` trigger for that channel.
  - **`POST /v1/connectors/slack/events`** — answers `200` with `{"event_id": "<EVENT_ID>", "replayed": false}` for an accepted mention, and `{"ignored": "<reason>"}` otherwise: `unreadable_body`, `bot_message`, `team_not_mapped`, or `unsupported_event`.
  - **Slack signature errors** — a bad signature returns `401` `UZ-WH-010` and a stale timestamp `UZ-WH-011`, as every signed route does. The error registry now marks `UZ-SLK-010`, `UZ-SLK-011`, and `UZ-SLK-020` retired.

  ## Bug fixes

  - **An answer with nowhere to go is no longer queued** — an answer from a scheduled run, a steer, or a GitHub review was queued for its model provider, which is not a place to post, and then dropped. Only an event with a reply destination, such as a Slack thread, now owes a delivery.

  ## CLI

  - **`agentsfleet install --library <LIBRARY_ID> --slack-channel <CHANNEL_ID>`** — attaches the new fleet to one Slack channel. An identifier that is not `C` or `G` followed by at least eight upper-case letters or digits fails before any request: `must be a Slack channel ID: C or G, then upper-case letters and digits`.
...
<Update label="Sep 21, 2026" tags={["API", "CLI", "Dashboard", "What's new", "Breaking changes"]}>
  ...
  You can now list the Fleet library entries your workspace onboarded and remove the ones you no longer want. Until now, every entry a workspace added stayed in the install gallery for good.

  Removing an entry leaves fleets installed from it alone. Installing copies the bundle onto the fleet, so the fleet keeps running from its own copy.

  ## Breaking changes

  - **`agentsfleet library add` is now `agentsfleet library create`** — every other collection on this surface uses `create`, so the library group now matches. There is no alias: `agentsfleet library add` is rejected as an unknown subcommand, so update any script that uses the old spelling. Run `agentsfleet library --help` for the current verbs.
  - **`agentsfleet connector status` is now `agentsfleet connector show`** — reading one resource is called `show` everywhere else.
  - **`agentsfleet auth status` is removed** — `agentsfleet whoami` answers for it. The two showed nearly the same thing, so `whoami` gained the one field it lacked: `source`, which tells you whether the credential in use came from the `environment` or from a `file`. That matters because an exported service key overrides a stored login, so a terminal can look signed in as one identity while acting as another. `agentsfleet doctor` still checks reachability and workspace binding.

  ## What's new

  - **`agentsfleet library list`** — lists the entries your workspace onboarded, with the identifier that `agentsfleet library delete` takes.
  - **`agentsfleet library delete <ENTRY_ID>`** — removes one entry. Running `agentsfleet library` on its own still prints the full gallery.
  - **A Library page in the dashboard** — under the workspace sidebar. It lists what the workspace owns, and names the entry in a confirmation before removing it.
  - **Removal is permanent** — nothing is kept behind. Onboard the same bundle again and it returns as a new entry with a new identifier.

  ## API reference

  - **`GET /v1/workspaces/{workspace_id}/library-entries`** — lists only the entries this workspace onboarded, never a platform entry, paged on its own cursor. The combined gallery at `GET /v1/workspaces/{workspace_id}/fleet-libraries` is unchanged, and `agentsfleet install --library` still resolves against it.
  - **`DELETE /v1/workspaces/{workspace_id}/library-entries/{entry_id}`** — removes one entry and returns `204`. It needs the `library:write` scope. Repeating the call returns `204`, and so does an identifier belonging to another workspace, so the reply never reveals whether an entry exists elsewhere.
  - **No new error codes** — a malformed `entry_id` returns `400`, a cursor from a different collection returns `UZ-LIBRARY-001`, and a database failure returns `UZ-LIBRARY-006`.

</Update>

<Update label="Sep 20, 2026" tags={["API", "CLI", "What's new", "Bug fixes"]}>
  ## Every charge records the fleet that incurred it

  Two fleets could share one `event_id`, because that identifier is minted per admission and nothing tied it to a fleet. The billing ledger settled a charge on `(event_id, charge_type)` alone, so two fleets' charges could pile into a single row: one fleet's spend counted against the other, with no report that would show it.

  Charges now settle on `(event_id, charge_type, fleet_id)`, and `fleet_id` can no longer be null.

  ## What's new

  - **`agentsfleet billing` groups charges by fleet and event** — when two of your fleets share an `event_id`, you see one row per fleet instead of two charges merged into one line.
  - **A charge always names its fleet** — `GET /tenant/charges` was documented as returning a null `fleet_id` for charges written before fleet identifiers survived deletion, but no database holds such a row. `workspace_id` and `fleet_name` are still nullable, and `fleet_name` is still a snapshot taken when the charge was written.

  ## Bug fixes

  - **A charge counts only its own fleet's spend** — `billing.usage_ledger` now settles on `uq_usage_ledger_event_id_charge_type_fleet_id`. Two fleets charged under one `event_id` get two rows, each with its own `credit_deducted_nanos`. The second fleet's charge used to be added to the first fleet's row.
  - **A continued run remembers what it resumed** — a run continued from an approval keeps its `resumes_event_id` whichever writer lands first. Both the lease path and the approval path write this row, and whichever one arrived without a predecessor used to erase the value already stored, leaving the history unable to show what the run followed from.
  - **A redelivered approval no longer announces the run twice** — when an approval lands on a row the lease path already opened, no second `event_received` is published, so a console counts the run once.
...
<Update label="Sep 19, 2026" tags={["API", "CLI", "What's new", "Bug fixes"]}>
  ## See who you are signed in as

  `agentsfleet whoami` tells you the person, tenant, and credential this computer is signed in with. Signing in saves a credential that carries no readable details of its own — the server works out what it can do from the record it names — so until now nothing on your machine could answer that question.

  ## Upgrading

  Update the server before the client. A client that calls `/v1/users/me` against a deployment that does not serve it yet gets a `404`, which says nothing about your credential, because the path is matched before the credential is checked. When that happens, `agentsfleet login` keeps the credential it just saved and tells you the deployment cannot answer, and `agentsfleet whoami` names the deployment instead of calling the credential good or bad. Every other command keeps working.

  ## What's new

  - **`agentsfleet whoami`** — prints `email`, `name`, `user_id`, `tenant`, `credential`, `api_url`, and `scopes`. `--json` returns the same fields for a script. It gains a `source` field on Sep 21, when it absorbs `agentsfleet auth status`. With nothing signed in, it points you at `agentsfleet login` and sends no request.
  - **`agentsfleet login` names the account it signed you in to** — the closing line reads `signed in as <name> — <tenant>` instead of `login complete`. It costs no extra request: the check that already ran after signing in now returns the identity.

  ## API reference

  - **`GET /v1/users/me`** — returns `user_id`, `email`, `display_name`, `tenant_id`, `tenant_name`, `credential`, and `scopes`. It accepts a browser session token, a tenant API key, or a command-line credential; a tenant API key answers with the person who created it. It needs no scope, so it also works as a check that a credential is still valid. A caller this tenant does not know as a person is refused with `403`.

  ## Bug fixes

  - **A credential without billing access is no longer reported as rejected** — signing in used to verify a credential by calling `GET /v1/tenants/me/billing`, which needs the `billing:read` scope. Anyone signed in without that scope was told the server had rejected their credential, when the server had refused that one path. The check now calls `GET /v1/users/me`, which needs no scope.
...
<Update label="Sep 19, 2026" tags={["API", "What's new", "Bug fixes"]}>
  ...
  Installing a fleet that declares a mintable credential now writes an approved integration grant and raises no approval card. Installing is the answer: you chose the fleet, its bundle names the integration, and `TRIGGER.md` names the repositories and the access level. A fleet declaring `repository_access: write` used to open a `repository_write` gate on every run, and because each model turn is its own event, a single instruction could raise a card per turn and post nothing.

  ## What's new

  - **The integration grant authorises a repository write** — one approved row in `core.integration_grants` per fleet and service, checked before any token is minted. `agentsfleet grant list --fleet <FLEET_ID>` shows it `approved` with an `approved_at` the moment the install returns.
  - **`agentsfleet grant delete <GRANT_ID> --fleet <FLEET_ID>` is the stop button** — the next event is refused before the fleet contacts the provider. It covers one fleet and one service, so it halts every repository that fleet reaches through that service; narrow a fleet's reach by editing `repositories:` instead.
  - **`budget.daily_dollars` bounds the spending** — it halts the fleet rather than one credential request, and it is now the only spending limit on a write path.

  ## Bug fixes

  - **A continuation no longer raises its own card** — each model turn carries a fresh event identifier, so a run that took ten turns asked ten times. It asks none.
  - **`repository_write` gates are no longer raised** — gates recorded before this release stay readable in the approval inbox, and an event already parked on one still resolves from it.
  - **Three error codes retired** — `UZ-REPAIR-010`, `UZ-REPAIR-011` and `UZ-REPAIR-013` can no longer be returned. A fleet without an approved grant returns `UZ-GRANT-001` instead. The retired codes keep their anchors, because older logs still carry them.
  - **The 32-request allowance is gone** — it was counted per approval, and with one approval per run a ten-turn run drew far more than the number the card showed. `budget.daily_dollars` replaces it.

</Update>

<Update label="Sep 19, 2026" tags={["CLI", "What's new", "Bug fixes"]}>
  ## The terminal can answer a fleet that is waiting on you

  `agentsfleet approvals` lists, reads, and decides approval gates. A fleet whose `TRIGGER.md` declares `repository_access: write` opens a gate on every run and parks until a person decides it, so `agentsfleet steer` on one of those fleets could not finish from a terminal before now.

  ## What's new

  - **`agentsfleet approvals`** — `list`, `show <gate_id>`, `approve <gate_id>`, and `deny <gate_id>`. The list prints `GATE`, `FLEET`, `KIND`, `STATUS`, and `ACTION`, and narrows with `--fleet <fleet_id>`. `show` prints the gate's blast radius in full, so the reach is read before it is granted.
  - **`agentsfleet library add`** — onboards a workspace library entry from `--github <owner/repo>`, `--from <path>`, or `--template <id>`. Exactly one source is required; none or several exits `4`. `--ref` pins a revision for `--github`, `--replace` overwrites an entry the workspace already holds, and the output names the entry's secrets, tools, and network hosts beside the install command to run next.
  - **`agentsfleet status` counts what is waiting** — a `Waiting` row joins Name, Status, Events, and Budget. When any fleet is parked, the block ends with the command that lists the gates.
  - **`agentsfleet secret list` is a table** — `NAME`, `KIND`, and `CREATED`, with a readable date in place of the raw counter the old two-field output printed. `--json` is unchanged.

  ## Bug fixes

  - **`agentsfleet library` lists the workspace gallery** — it read the platform-only catalogue, so an entry a workspace owned was installable but invisible. It now reads the same list `install --library` resolves against, and a `TIER` column separates `platform` from `tenant`. A workspace with no entries says so and names `agentsfleet library add`.
  - **Server failures read as sentences** — a refused install prints the sentence written for a reader, such as "That Fleet Bundle isn't valid. It's missing `SKILL.md`, or has an unsafe or oversized file. Check the source and try again.", in place of the terse internal detail.
  - **`agentsfleet logout` claims only what it does** — its help now reads: revoke this machine's credential, abort unfinished sign-ins, and clear local state. Other machines stay signed in, and it never claimed to reach tokens already minted.
  - **The paging hint names a real command** — a paged `agentsfleet list` points at `agentsfleet list --starting-after <cursor>`. It named `agentsfleet fleet list`, which does not exist.
  - **One spelling for the library placeholder** — help and hints advertise `--library <library_id>` throughout, including `install --help`.
</Update>

<Update label="Sep 16, 2026" tags={["API", "Bug fixes"]}>
  ## A quiet fleet stops reading "Connecting…"

  `GET /v1/workspaces/{workspace_id}/fleets/{fleet_id}/events/stream` opens with a `hello` frame as soon as the subscription attaches. A fleet with nothing happening sent nothing until the first heartbeat fifteen seconds later, so a surface that reports itself live off the first frame showed "Connecting…" while nothing was wrong.

  ## Bug fixes

  - **`hello` on a fleet's stream** — `event: hello` carrying `{"kind":"hello","fleet_ids":["<fleet_id>"],"counters":{}}`, naming the one fleet this connection serves. The workspace stream has always opened this way; the per-fleet route is what lacked it.
  - **The sequence is undisturbed** — the frame uses identifier `0` and does not advance the activity sequence, so a client may report the stream live without waiting for a heartbeat.
  - **When no `hello` arrives** — an instance holding no publish/subscribe connection sends none. The route still serves; it has nothing to say hello about.
</Update>

<Update label="Sep 16, 2026" tags={["API"]}>
  ## A repeated report charges nothing

  `POST /v1/runners/me/reports` takes the lease id as its idempotency key, so it accepts no `Idempotency-Key` header. Send the same report again after a lost response and the stored outcome comes back.

  ## What's new

  - **The lease id is the key** — one lease carries one terminal result, so the identifier already in the request is what makes a repeat safe. Nothing extra to send.
  - **A repeat costs nothing** — the second call returns the first call's stored outcome, charges nothing, and counts the run once.
  - **A superseded holder still writes nothing** — a report from a holder the fleet has already replaced is refused by the fence, so a stale writer cannot land a partial finalize on the current holder's run.
</Update>

<Update label="Sep 16, 2026" tags={["API"]}>
  ## A retried steer runs once

  `POST /v1/workspaces/{workspace_id}/fleets/{fleet_id}/messages` accepts an optional `operation_id`. Send the same value on a retry and the fleet runs once and charges once, however many times the request arrives.

  ## What's new

  - **`operation_id`** — an optional, nullable string on the steer body, your own name for the operation, repeated across its retries. A retry carrying a value already seen is answered with the first attempt's event rather than starting a second run.
  - **Why the caller supplies it** — a timeout does not prove the request failed, and the server cannot tell a retried `POST` from a person pressing send twice, because the bytes are identical. Only the caller knows which one it is.
  - **Leaving it out** — omit it and every message is its own operation, so two identical messages start two runs. That is what a person pressing send twice expects, and it is why the field is optional rather than required.
</Update>

<Update label="Sep 16, 2026" tags={["Bug fixes", "API"]}>
  ## One fleet's broken configuration stops that fleet, not your runners

  A fleet whose stored configuration cannot be read now ends its own event and leaves every other fleet leasable. Before, a runner poll that landed on such a fleet answered `500` `UZ-INTERNAL-003`, so one unreadable document could fail polls across a whole deployment.

  ## Bug fixes

  - **Runner polls** — a fleet with an unreadable configuration answers `200` with `{"lease": null, "retry_after_ms": 1000}`, the same empty answer a poll gets when no fleet holds work, instead of `500` `UZ-INTERNAL-003`. The runner waits out the hint and polls again.
  - **`config_unreadable`** — a new `failure_label` on the refused event, returned beside `failure_detail`, so the fleet needing attention is named rather than inferred from a log.
  - **Refused events close** — the refusal writes the terminal `gate_blocked` row and publishes the closing bracket, so a watcher on that fleet's stream sees the event end instead of hanging.
</Update>

<Update label="Sep 14, 2026" tags={["UI", "Bug fixes"]}>
  ## Chat keeps your place while work continues

  Sending a message moves Chat to your new turn. A paw and **Working…** appear beside the fleet's reply until it starts answering.

  ## What's new

  - **Message field** — longer drafts expand the field up to its height limit, then scroll inside it.
  - **Jump to latest** — the button stays above the message field when you read older turns.

  ## Bug fixes

  - **Reading position** — new background activity no longer pulls you away from older messages.
  - **Fleet tabs** — scroll sideways so every tab remains reachable on a narrow screen.
  - **Status line** — figures wrap, and a long outcome shortens to fit the available width.
</Update>

<Update label="Sep 14, 2026" tags={["Bug fixes", "UI"]}>
  ## Quiet chats renew their connection automatically

  Brief connection renewals no longer interrupt a quiet chat with a reconnect warning.

  ## Bug fixes

  - **Connection recovery** — long-lived quiet connections no longer accumulate failures across routine renewals.
  - **Silent failures** — stalled connection attempts time out. Chat detects connections without activity or a server heartbeat after 45 seconds of silence and retries automatically.
  - **Connection notice** — sustained failures explain that automatic retries continue. **Retry now** starts an immediate attempt when you want one.
  - **Overlapping recovery** — callbacks from a replaced connection cannot close its replacement or change the conversation.
</Update>

<Update label="Sep 12, 2026" tags={["UI", "Website", "Bug fixes"]}>
  ## The credit balance shows on every page

  Every dashboard page now shows what is left to spend. A **Credits** chip sits in the header beside the workspace switcher and links to **Settings → Billing**. When the balance is exhausted the chip turns red, because that is the state that blocks the next run.

  - **Precision** — the chip rounds to cents. A balance under one cent keeps four decimals, and a smaller one reads `<$0.0001`. A live balance never shows as `$0.00`.
  - **Freshness** — the figure is rendered with the page rather than polled. A tab left open overnight shows last night's figure; **Settings → Billing** holds the live one.
  ...
  - **Fleet library import** — importing a bundle from GitHub now has 30 seconds instead of 10. A repository that GitHub served slowly failed in the dialog while `agentsfleetd` was still fetching it. The two pages that host the action declare a 60 second server ceiling, so the dialog's message arrives instead of a gateway `504`.
  - **Balance card caption** — the billing balance card drops its `spent $X · N events` caption. It summed only the charge rows the page had loaded, so page two reported a different figure for the same account.
  - **Landing page headings** — five sections opened with a 12px eyebrow marked as a heading, above their real heading. A screen reader heard ten headings where there are five sections.
...
<Update label="Sep 11, 2026" tags={["API", "UI"]}>
  ## Every live frame carries the fleet's counters

  Each frame a fleet publishes on its live tail now carries `events_processed` and `budget_used_nanos` as an absolute snapshot, and the workspace stream's `hello` carries a `counters` map keyed by fleet id. Both counters only ever grow, so the Fleets wall keeps the greater of the figure it holds and the figure a frame carries, and never adds: a dropped, duplicated, late or out-of-order frame cannot leave a tile wrong, and a gap in the stream is followed by a fresh `hello`. A tile is correct before its first event: the `hello` reports where each fleet stands at subscribe time.

  - **Frames** — `event_received`, `event_complete`, `gate_opened` and `gate_resolved` all carry the pair. A frame the daemon could not fill omits both fields rather than sending zeros; a client leaves the figures it has standing.
  - **`hello`** — `{"kind":"hello","fleet_ids":[…],"counters":{"<fleet_id>":{"events_processed":n,"budget_used_nanos":n}}}`. A fleet that has never run answers with zeros; a fleet the read could not answer for is omitted.
  - **Wall** — the tile footer no longer derives its figures from the streamed rows, and the wall no longer re-reads the fleet list when a row arrives without a price. A run that parks behind an approval shows the event it counted.
  ...
  - **Buttons** — one primary action per surface renders the default variant; every other non-destructive action is `ghost`.
  - **Runs strip** — a processed run whose list row carries no body reads `Completed.`; only the event detail, which holds the body, says when no reply was recorded.
  - **Admin leases** — the Fleet column names the agent by callsign, as every other agent column does, and keeps the fleet id on hover.
</Update>

<Update label="Sep 08, 2026" tags={["API", "UI", "Bug fixes"]}>
  ## Connecting GitHub completes again

  Pressing **Connect** on GitHub now finishes and the connector reads **Connected**. Connect authorized the account and obtained its grant, then failed while asking GitHub which App installations the account can reach; the connection was never written.

  A refusal at that step now answers `502` with `UZ-CONN-009`, separately from the token exchange. The two shared `UZ-CONN-006`, so a connect that had already obtained its grant reported a failed exchange and pointed at credentials that were working.
...
<Update label="Sep 07, 2026" tags={["Internal", "Performance"]}>
  ## Four benchmark lanes measure what steer, lease and delivery sustain

  `make bench-steer`, `make bench-lease`, `make bench-outbound` and `make bench-cardinality` drive the production paths against the compose Postgres and Redis and write one JSON result each under `bench/results/`. `make bench-compare LANE=<lane> PROFILE=rig` prints the delta against the committed baseline in `bench/baselines/` and always exits `0` — no build fails on a throughput number.

  - **Profiles** — `PROFILE=rig` is unbounded against the compose stack; `dev` and `prod` carry caps and refuse a parameter above them before opening a connection. `prod` also refuses without `BENCH_LOAD_PRODUCTION=i-accept-the-blast-radius`.
  - **Attribution** — every result carries Redis commands and Postgres round trips separately, read from the servers' own counters, so a saturation is attributed rather than guessed.
  ...
  - **Honest zeros** — a lane refuses a knob it cannot parse, writes no tail for an empty distribution, and records a window that ran out as an abort; `bench-compare` names differing parameters before any delta.
  - **Cardinality** — about 4.6 KB of Redis per idle fleet, flat from 10 to 10 000 fleets; a million idle fleets is roughly 4.6 GB.
  - **Sweep** — a run creates only under its own prefix and removes everything it created on every exit path, reporting `created` against `swept` in the result.
</Update>

<Update label="Sep 06, 2026" tags={["API", "Bug fixes"]}>
  ## Session verification bounds stale signing keys

  Browser session verification stops using signing keys more than six hours and fifteen minutes after their last successful refresh. A provider outage beyond that ceiling returns `503` with `UZ-AUTH-004`; verification recovers when a refresh succeeds.

  ## Upgrading

  - **Fleet versions** — use stable `MAJOR.MINOR.PATCH` values. Import now refuses prerelease and build suffixes that installation cannot accept.
  ...
  - **Malformed URL escapes** — memory paths reject escapes such as `%+a` instead of decoding them as bytes.
  - **Extreme dates** — timestamps beyond the supported calendar range display their original millisecond value as text.
</Update>
