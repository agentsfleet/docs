...
<Update label="Sep 21, 2026" tags={["API", "CLI", "Dashboard", "What's new"]}>
  ...
  - **`agentsfleet library list` and `agentsfleet library remove <ENTRY_ID>` are the terminal half** — `list` prints the identifier `remove` takes. Bare `agentsfleet library` still prints the full gallery, unchanged.
