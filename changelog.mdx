...
<Update label="Apr 29, 2026" tags={["Breaking", "API", "CLI"]}>
  ...
  See [Authoring skills](/fleets/authoring) for the canonical shape.
...
<Update label="Apr 19, 2026" tags={["Improvements", "CLI"]}>
  ...
  The CLI and API now speak one language: `zombiectl install → up → status → logs → kill`. `zombiectl --help` is shorter, the API surface is tighter, and the docs, product, and code all describe the same thing. See [Agents](/fleets/overview).
</Update>

<Update label="Apr 18, 2026" tags={["Docs"]}>
  ## Docs reshaped around the agent lifecycle

  A new [Agents](/fleets/overview) section walks through installing a template, adding credentials, running, observing, and killing an agent. Pages describing the legacy v1 pipeline have been retired; the old `/specs/*` and `/runs/*` URLs now 404.

  New pages: [overview](/fleets/overview), [install](/fleets/install), [running](/fleets/running), [credentials](/fleets/secrets), [webhooks](/fleets/webhooks), [skills](/fleets/authoring), [templates](/fleets/library).
</Update>
...
<Update label="Apr 18, 2026" tags={["New", "Security", "Integrations"]}>
  ...
  Seven providers ship first-class: **agentmail**, **Grafana**, **Slack**, **GitHub**, **Linear**, **Jira**, and **Clerk** (via Svix). Onboarding takes one field in `TRIGGER.md`; secrets are workspace-vaulted and rotate without an agent redeploy. See [Webhooks](/fleets/webhooks).
</Update>
