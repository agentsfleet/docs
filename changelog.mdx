...
<Update label="Jul 29, 2026" tags={["Breaking", "What's new", "API", "CLI", "UI"]}>
  ...
  - **Secrets stored before Jul 11, 2026 need replacing** — the vault reads only the current envelope format. A secret sealed under the earlier format answers a typed unsupported-version error at use. Replace it once with `agentsfleet secret update` (or delete and create it) and it is re-sealed in the current format.
  ...
  - **The dashboard's Edit dialog is the Create form, prefilled** — provider, base URL, and model come from the row you opened; you supply the key. A custom endpoint's base URL is editable after creation for the first time. The secret writes first and exactly once, so a failed save reports one outcome instead of leaving the table half-updated.
  ...
  - **A failed save in Edit no longer half-applies** — the model rename used to commit before a failed key write, leaving the table renamed while the credential kept the old key.
