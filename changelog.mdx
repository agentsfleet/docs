...
<Update label="Jul 26, 2026" tags={["What's new", "Bug fixes", "Performance"]}>
  ...
  - **Stopping or deleting a fleet clears its readiness entry** — a stopped fleet's entry was unreachable and held a slot of the bounded sample permanently. A stop keeps the fleet's stream, so a resume still finds its undelivered event.
