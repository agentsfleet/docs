...
<Update label="Jun 01, 2026" tags={["What's new", "Internal"]}>
  ...
  The follow-up promised in the May 27, 2026 runner-fleet release is here: an event that runs longer than the lease window is no longer reclaimed and re-run. While a runner is executing an event, it renews the lease in the background, pushing the kill deadline forward so a single long-running agent runs to completion instead of being cut off after 30 seconds. A hard 12-hour ceiling still caps a runaway run.

  ## What's new

  - **Background lease renewal.** A runner renews mid-execution, extending both its lease and its work affinity in one atomic step. The renewal is guarded by the lease's fencing token, so a runner that already lost the lease cannot extend it.
  - **Fail-safe renewal.** A renewal that hits a transient database fault is retried on the next tick rather than ending the run; only a genuinely missing or superseded lease stops it, so a long event survives a brief datastore blip.
  - **Bounded runtime.** Renewal cannot extend a single run past a 12-hour maximum, so a stuck agent is still reclaimed.
</Update>
