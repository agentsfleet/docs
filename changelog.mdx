...
<Update label="Aug 13, 2026" tags={["Breaking", "Bug fixes", "API"]}>
  ...
  - **`GET /v1/tenants/me/billing` no longer returns `free_trial`.** The response carries `balance_nanos`, `is_exhausted`, and `exhausted_at`, which is the whole billing state a client needs. The member is gone from the published schema and from its `required` list, so a client that reads it must stop.
