...
<Update label="Jul 08, 2026" tags={["UI", "API"]}>
  ...
  - **Add a model without re-pasting a key** — the "Add model" dialog's Known-provider tab offers "use existing key" alongside pasting a new one, so a second or third model on the same provider account reuses the stored key.
