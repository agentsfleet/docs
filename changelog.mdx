...
<Update label="Sep 19, 2026" tags={["API", "What's new", "Bug fixes"]}>
  ...
  - **`agentsfleet grant revoke` is the stop button** — the next event is refused before the fleet contacts the provider, so revoking costs nothing and takes effect immediately.
