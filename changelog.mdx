...
<Update label="May 17, 2026" tags={["Performance", "Observability"]}>
  ...
  - **`REDIS_POOL_MAX_IDLE`** (default `8`) — concurrent in-flight short-lived commands per process. Operators raise it for sustained high-concurrency workloads; the request-path completes in single-digit ms even over Upstash TLS, so `> 16` is unusual.
