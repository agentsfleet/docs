...
<Update label="May 18, 2026" tags={["Breaking", "CLI"]}>
  ...
  - **On by default.** Fresh installs bootstrap `~/.config/zombiectl/telemetry.json` with `consent: "granted"` on the first invocation. No interactive prompt — the contract is the `--help` env-var section + the [Configuration → Telemetry](/cli/configuration#telemetry) docs page.
