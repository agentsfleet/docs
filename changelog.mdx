...
<Update label="Sep 19, 2026" tags={["API", "CLI", "What's new", "Bug fixes"]}>
  ...
  - **`agentsfleet whoami`** — prints `email`, `name`, `user_id`, `tenant`, `credential`, `api_url`, and `scopes`. `--json` returns the same fields for a script. With nothing signed in, it points you at `agentsfleet login` and sends no request.
  ...
  - **`GET /v1/users/me`** — returns `user_id`, `email`, `display_name`, `tenant_id`, `tenant_name`, `credential`, and `scopes`. It accepts a browser session token, a tenant API key, or a command-line credential; a tenant API key answers with the person who created it. It needs no scope, so it also works as a check that a credential is still valid. A subject with no user record is refused with `UZ-AUTH-001`.
