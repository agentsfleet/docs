...
<Update label="Jun 04, 2026" tags={["Breaking", "What's new", "UI", "CLI", "Security"]}>
  ...
  - **`zombie-runner register` and its `--token` / `--host-id` flags are gone.** The runner binary no longer takes an identity credential; it runs on the installed `ZOMBIE_RUNNER_TOKEN`.
</Update>
