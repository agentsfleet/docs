...
<Update label="Jul 27, 2026" tags={["Breaking", "What's new", "Bug fixes", "API", "CLI"]}>
  ...
  - **The gallery summary no longer carries `support_files`** — read `GET /v1/workspaces/{workspace_id}/fleet-libraries/{tier}/{id}` for the support-file manifest. Nothing else moved off the card.
  ...
  - **One Fleet library entry, in full** — the new detail route returns an entry's requirements, per-credential reason copy, and its support-file manifest.
  - **A Fleet library entry in another workspace is indistinguishable from one that does not exist** — both answer `404` with `UZ-LIBRARY-007`, so the route cannot be used to discover which ids exist elsewhere.
