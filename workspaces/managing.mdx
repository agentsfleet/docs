...
2. List local workspace entries.
   ...
   ```text
   NAME             WORKSPACE                               STATUS    AGO
   platform-ops     <varies>                                active    41d
   ```

   `STATUS` reads `active` for the workspace you are working in. `AGO` is how
   long ago each one was created.
