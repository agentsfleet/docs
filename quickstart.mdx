...
<Step title="Install and run the install skill">
  Two ways to drop the `usezombie-install-platform-ops` skill into your host's skills folder. Pick one.
  ...
  <Tabs>
    ...
    <Tab title="curl (one command)">
      Installs `zombiectl` and the skill in one shot (requires Node — it runs npm under the hood). If you skipped the steps above and ran this on a fresh machine, you still need `zombiectl login` (Step 3) before the skill works. Want to read it first? `curl -fsSL https://usezombie.sh` without `| bash` prints the script.
