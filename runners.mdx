...
- **Live replies need a current runner.** A runner older than `agentsfleet-runner` 0.51.0 still completes runs, but the dashboard shows each reply only when its run finishes. Update runners along with `agentsfleetd`.
