...
The first is the admin state. You set it with the actions below; it never changes on its own.

| Admin state | Meaning |
|---|---|
| `active` | Takes work. The normal state. |
| `cordoned` | Takes no new work. Its running work stays fenced until it expires or moves. |
| `draining` | Takes no new work and finishes what it has. |
| `drained` | Finished draining. Idle until you change its state. |
