...
## How tools, secrets, and the network allowlist combine

Tools are the *what* (verbs the fleet may invoke). Secrets are the *what-with* (named secrets the tool bridge substitutes into outbound calls, referenced by the `credentials:` list in `TRIGGER.md` — see [Secrets & ENVs](/fleets/secrets)). The network allowlist is the *to-where* (hostnames the sandbox firewall lets traffic reach).
...
Drop any one and the call fails: missing tool → fleet can't even try; missing secret → tool bridge has nothing to substitute; missing host → firewall drops the request.
...
- **Typos pass parsing.** Declaring `http_requesst:` (typo) won't fail onboarding or `agentsfleet fleet update --from`. Catch this by tailing `agentsfleet logs <fleet_id>` after the first event and looking for the skip warning, or by spotting tool calls in the activity stream that don't match what your `SKILL.md` prose expects.
...
- [Secrets & ENVs](/fleets/secrets) — adding the secrets the tool bridge substitutes.
