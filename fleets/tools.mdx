...
23 tools are user-callable today. They group into nine categories below. For the file format, validation rules, and frontmatter, see [Authoring a fleet](/fleets/authoring).
...
Cross-run durable state. See [Memory](/memory) for the four-category model and key conventions.

| Tool | What it does | Reach for when |
|---|---|---|
| `memory_store` | Upsert a distilled fact (key + category + body + tags). | End of every run where the fleet learned something worth keeping. |
...
The fleet does not get a free shell or filesystem inside the sandbox. The one file tool exposed to fleets enforces hash-anchored edits to prevent silent overwrites.

| Tool | What it does |
|---|---|
| `file_edit_hashed` | Replace lines in a file using Hashline anchors. Edits are rejected if the on-disk content has drifted from the hash the fleet saw — protects against concurrent-edit races. |
...
When the fleet tries to invoke a tool not in the fleet's resolved `tools:` set, the underlying fleet runtime returns a "no such tool" error to the fleet in-prompt — visible in the activity-stream entry, not as a wire response. The platform registers `UZ-TOOL-004` (Tool not attached) for this case but the v2 emission path stays inside the activity stream. See [Error codes → Tool](/api-reference/error-codes#tool).
