...
Tools are the *what* (verbs the fleet may invoke). Secrets are the *what-with* (named secrets the tool bridge substitutes into outbound calls, referenced by the `credentials:` list in `TRIGGER.md` — see [Secrets](/fleets/secrets)). The network allowlist is the *to-where* (hostnames the sandbox firewall lets traffic reach).
...
- [Secrets](/fleets/secrets) — adding the secrets the tool bridge substitutes.
