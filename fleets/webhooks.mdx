...
| `UZ-WH-020` | The trigger's `source` is not a supported provider, or the workspace has no secret for it. | Use a supported `source`. Create the workspace secret named by `credential_name`, which defaults to the `source` name. |
