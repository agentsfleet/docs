...
Install first. A grant belongs to a fleet, so the fleet has to exist before its
...
3. Name your repositories.

   A library entry ships its own `TRIGGER.md` and cannot know your repository,
   so `--library` alone leaves the fleet subscribed to nothing. Edit `TRIGGER.md`
   in a local bundle as shown above, then push it to the installed fleet.

   ```bash
   agentsfleet fleet update <FLEET_ID> --from ./my-bundle
   ```
...
4. Approve the fleet's integration grant.

   Each fleet needs its own approved grant before it receives events. The fleet
   appears in **Settings → Integrations** once it is installed.
