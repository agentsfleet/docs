...
`agentsfleet credential add <name> --data=@-` stores a JSON-shaped secret in the current workspace's vault. The name is what you reference from `TRIGGER.md`.
...
```bash
cat <<'JSON' | op inject | agentsfleet credential add github --data=@-
{
...
- `--provider openai-compatible` opts the credential into the custom-endpoint path; `--base-url` is then **required** and is valid only here.
- `--model` is **required** — every self-managed credential names the model it serves.
- `--api-key` is **optional** for a custom endpoint (a keyless gateway dials with no key); a named provider still requires one.
- The typed flags and `--data` are mutually exclusive — a credential is either a model provider or a raw-JSON tool secret, not both.
...
`agentsfleet credential list` returns the names and creation timestamps of every credential in the current workspace. **Values are never returned.** This is by design — once stored, a secret is only ever used by the firewall.
...
A fleet declares the credentials it needs as a **list of bare key names** in its `TRIGGER.md` frontmatter. Nothing else — no URI scheme, no path, no secret-manager-specific prefix. The names are looked up verbatim against the workspace vault.
...
Rotating a credential is a single command — no re-install is needed. The next event handled by any fleet using that credential picks up the new value.

```bash
agentsfleet credential add github --data=@- --force <<'JSON'
...
`agentsfleet credential show <name>` prints metadata (creation timestamp, last-rotated) but **never the value**. The vault is one-way: write at the boundary, read only by the firewall.
