...
The dashboard waits 30 seconds for an import, then reports a timeout in the dialog. `agentsfleetd` allows its own GitHub fetch 60 seconds, so a large bundle over a slow link can outlast that wait. Reload the library to see whether the entry arrived, then import again.
