...
`UZ-VAULT-005` means the workspace already holds that name. Creating a secret claims a name that is free. It never replaces a stored value, so nothing is written and the command exits 0.

To replace a value in place, send `PATCH /v1/workspaces/{workspace_id}/secrets/{secret_name}` with the new `data`. The name stays claimed for the whole call, so fleets that require it keep running.

The client has no rotate command yet. Deleting and creating through the client leaves the name absent between the two calls, so any fleet that requires it fails during that gap. See [Remove or undo](#remove-or-undo).
...
This delete-then-create pair also rotates a value, at the cost of an outage. Prefer `PATCH /v1/workspaces/{workspace_id}/secrets/{secret_name}`, which replaces the value without releasing the name. There is no overwrite flag on `create`.
