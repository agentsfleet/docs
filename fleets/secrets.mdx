...
3. List secret names.
   ...
   ```text
   NAME    KIND      CREATED
   github  <varies>  <varies>
   ```

   `CREATED` reads as a date and time rather than a raw counter. `--json`
   returns the fields it always has, so a script reading it needs no change.
