...
4. When a connected GitHub App sends nothing, check the subscription.
   ...
   which repositories it wants. A fleet that names none subscribes to none, and
   `agentsfleet events` stays empty because no run was ever created.

   ```bash
   agentsfleet get <FLEET_ID> --json
   ```
   ...
   The fleet's `TRIGGER.md` needs `repositories` beside `source: github`, spelled
   `owner/repository`. Case does not matter. The fleet also needs an approved
   integration grant for the provider, which **Settings → Integrations** shows.
