...
Templates are markdown — commit `my-fleet/` to git, clone it on a new machine, and push it to an installed fleet with `agentsfleet fleet update <fleet_id> --from .`. Credentials never live in the template; they live in the workspace vault. The template is safe to share publicly; credentials are not.
