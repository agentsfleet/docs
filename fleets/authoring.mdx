...
Installing a [library entry](/fleets/library) with `agentsfleet install --library <id>` gives you a fleet backed by both files. This page is the reference for authoring your own — which you then onboard as a template (dashboard or API) before installing by id.
...
`TRIGGER.md` carries the runtime contract: trigger source, allowed tools, vault secret references, network allowlist, and budget caps. Everything that controls security, cost, or scheduling lives here. The body below the frontmatter is operator-facing prose (secret shapes, budget rationale, etc.) — the runtime does not parse it.
...
# Operator commentary about secret shapes, budget reasoning,
...
Each entry under `credentials:` is a **bare vault key name** referencing a JSON-shaped secret in the workspace vault (e.g. `github` carrying `{api_token, webhook_secret}`, `slack` carrying `{bot_token}`). Names like `github`, `slack`, `linear`, `jira` are secret references — **not** tool names. The tool bridge resolves the named secret at `http_request` time and substitutes its fields into outbound calls; the fleet sees the response, never the raw secret. Add and rotate via [Secrets & ENVs](/fleets/secrets).
...
Once onboarded, install the entry by id with `agentsfleet install --library <id>`. To re-parse and validate edits against an already-installed fleet, push them with `agentsfleet fleet update <fleet_id> --from <dir>` — the same parser runs and returns the same `UZ-AGT-*` codes.
...
<CardGroup cols={2}>
  <Card title="Fleet library" icon="layer-group" href="/fleets/library">
